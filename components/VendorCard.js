...
// Firebase initialization - reusing the same config as in other components
...
try {
  ...
  app = initializeApp(firebaseConfig, "vendor-card-component");
}
...
const VendorCard = ({
  ...
  // State for login message overlay
  ...
  const handleProfileClick = (e) => {
    ...
    // If user is logged in, allow default behavior (follow link)
    return true;
  };
  ...
  // Handle closing the login message
  const handleCloseLoginMessage = () => {
    setShowLoginMessage(false);
  };
  
  // Handle opening auth modal from login message
  ...
  const LoginMessageOverlay = ({ onClick, onClose }) => {
    return (
      <div className="fixed inset-0 z-50 overflow-y-auto overflow-x-hidden flex items-center justify-center bg-black bg-opacity-50">
        <div 
          className="bg-white rounded-lg shadow-xl max-w-md w-full mx-4 overflow-hidden cursor-pointer"
          onClick={onClick}
        >
          <div className="bg-blue-50 border-l-4 border-blue-400 p-4">
            <div className="flex items-center">
              ...
              <div className="ml-3 flex-grow">
                <p className="text-sm text-blue-700">Prašome prisijungti norint žiūrėti profilį</p>
              </div>
            </div>
          </div>
          ...
          <div className="p-4 bg-gray-50 text-center">
            ...
            <button 
              className="ml-2 px-4 py-2 border border-gray-300 rounded-md text-gray-700 hover:bg-gray-100 transition-colors"
              onClick={(e) => {
                e.stopPropagation();
                onClose();
              }}
  ...
  const renderPortals = () => {
    ...
    return (
      <>
        ...
        {showLoginMessage && ReactDOM.createPortal(
          <LoginMessageOverlay 
  ...
  return (
    <>
      ...
      >
        ...
        {/* Profile Button - Modified to check login status and show auth modal if needed */}
        {showProfileButton && (
          <motion.button
            onClick={(e) => {
              e.preventDefault();
              if (user) {
                ...
                window.location.href = `/tiekejai/${encodeURIComponent(name.toLowerCase().replace(/\s+/g, '-'))}`;
              } else {
      ...
      {/* Render portals for auth modal and login message */}
      {renderPortals()}
    </>
  );
};
